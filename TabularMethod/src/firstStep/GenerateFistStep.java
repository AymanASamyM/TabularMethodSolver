...
public class GenerateFistStep implements FirstStepInterface {
	...
	public void generateTabular(String minterms, String dontcares) {
		...
		while (i < Inputs.num) {
			Column x = new Column();
			Group tmp;
			for (int j = 0; j < columns.get(i).colGroups.size() - 1; j++) {
				tmp = Group.merge(columns.get(i).colGroups.get(j), columns.get(i).colGroups.get(j + 1));
	...
	public LinkedList<Implicant> getImplicants() {
		for (int i = 0; i < columns.size(); i++) {
			...
			for (int j = 0; j < columns.get(i).colGroups.size(); j++) {
				tmp = columns.get(i).colGroups.get(j).getPrimeImplicants();
				if (tmp != null) {
